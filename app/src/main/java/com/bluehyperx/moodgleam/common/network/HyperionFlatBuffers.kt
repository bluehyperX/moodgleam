...
class HyperionFlatBuffers(
    ...
    init {
        ...
        mSocket!!.connect(InetSocketAddress(mHost, port), TIMEOUT)
        register()
    }
    ...
    override fun getConnectedDeviceName(): String? = mHost

    override fun setRawLedData(leds: Array<ColorRgb>, priority: Int) {
        val rgbBytes = ByteArray(leds.size * 3)
        var idx = 0
        for (led in leds) {
            rgbBytes[idx++] = led.red.toByte()
            rgbBytes[idx++] = led.green.toByte()
            rgbBytes[idx++] = led.blue.toByte()
        }
        try {
            setImage(rgbBytes, leds.size, 1, priority)
        } catch (_: IOException) {}
    }
