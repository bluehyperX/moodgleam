...
class DeviceDetector {
    ...
    companion object {
        ...
        private val WIZ_PORTS = listOf(38899)
        ...
        /**
         * Checks if device is Philips WiZ
         */
        private fun detectWiz(host: String): DeviceInfo? {
            try {
                val socket = java.net.DatagramSocket()
                socket.soTimeout = CONNECTION_TIMEOUT_MS
                val address = java.net.InetAddress.getByName(host)
                val data = "{\"method\":\"getSystemConfig\",\"params\":{}}".toByteArray()
                val packet = java.net.DatagramPacket(data, data.size, address, 38899)
                socket.send(packet)
