 ...
 */
class DeviceScanner(
    private val targetDeviceType: DeviceDetector.DeviceType? = null,
    private val onDeviceFound: ((DeviceDetector.DeviceInfo) -> Unit)? = null,
) {
    private var ipsToTry: Array<String> = emptyArray()
    ...
    init {
        if (targetDeviceType == DeviceDetector.DeviceType.WIZ) {
            Log.d(TAG, "Targeting WiZ. Attempting instant UDP broadcast...")
            val broadcastDevices = performSyncWizBroadcastDiscovery()
            if (broadcastDevices.isNotEmpty()) {
                Log.i(TAG, "Instant broadcast found ${broadcastDevices.size} bulbs. Skipping slow scan.")
                foundDevices.addAll(broadcastDevices)
                broadcastDevices.forEach { onDeviceFound?.invoke(it) }
                // Leave ipsToTry empty to finish instantly
            } else {
                Log.w(TAG, "Instant broadcast failed. Falling back to slow subnet scan.")
                ipsToTry = getIPsToTry()
            }
        } else {
            ipsToTry = getIPsToTry()
            Log.d(TAG, "General scan. Initialized with ${ipsToTry.size} IPs")
            // Concurrently run broadcast as a bonus for general scans
            Thread { runWizBroadcastDiscovery() }.start()
        }
    }

    private fun performSyncWizBroadcastDiscovery(): List<DeviceDetector.DeviceInfo> {
        val devices = mutableListOf<DeviceDetector.DeviceInfo>()
        try {
            val socket = DatagramSocket()
            socket.broadcast = true
            socket.soTimeout = 1500 // Balanced timeout for reliable discovery
            val data = "{\"method\":\"getSystemConfig\",\"params\":{}}".toByteArray()
            val packet = java.net.DatagramPacket(data, data.size, InetAddress.getByName("255.255.255.255"), 38899)
            
            // Send multiple broadcast packets to ensure they get through
            repeat(3) {
                socket.send(packet)
                Thread.sleep(50)
            }
            ...
            while (System.currentTimeMillis() < endTime) {
                try {
                    ...
                    if (json.has("result")) {
                        ...
                        val name = result.optString("moduleName", "WiZ Bulb")
                        val deviceInfo = DeviceDetector.DeviceInfo(
                            host = host,
                            port = 38899,
                            type = DeviceDetector.DeviceType.WIZ,
                            protocol = "wiz",
                            name = name,
                            hostname = null // Skip hostname lookup for speed in broadcast phase
                        )
                        
                        if (devices.none { it.host == deviceInfo.host }) {
            ...
            Log.e(TAG, "Sync WiZ discovery error: ${e.message}")
        }
        return devices
    }

    private fun runWizBroadcastDiscovery() {
        try {
            val socket = DatagramSocket()
            socket.broadcast = true
            ...
            while (true) {
                ...
                try {
                    ...
                    if (json.has("result")) {
                        ...
                        val alreadyFound = foundDevices.any { it.host == deviceInfo.host }
                        if (!alreadyFound) {
                            foundDevices.add(deviceInfo)
                            onDeviceFound?.invoke(deviceInfo)
                            Log.d(TAG, "Broadcast discovered WiZ device: $name at $host")
                        }
                    }
                } catch (e: Exception) {}
            }
     ...
     * Проверяет TCP 19400 (Hyperion), UDP 4048, 19446 (WLED) и UDP 38899 (WiZ)
     * @return Triple<hasHyperionPort, hasWledPort, hasWizPort>
     */
    @WorkerThread
    private fun checkDevicePorts(host: String): Triple<Boolean, Boolean, Boolean> {
        var hasHyperionPort = false
        var hasWledPort = false
        var hasWizPort = false
        ...
        try {
            val datagramSocket = DatagramSocket()
            datagramSocket.connect(InetAddress.getByName(host), 38899)
            datagramSocket.close()
            hasWizPort = true
        } catch (e: Exception) {
        }
    ...
    fun tryNext(): DeviceDetector.DeviceInfo? {
        ...
        val (hasHyperionPort, hasWledPort, hasWizPort) = checkDevicePorts(ip)

        if (!hasHyperionPort && !hasWledPort && !hasWizPort) {
