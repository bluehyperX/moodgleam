...
**Чёрное превью на 4K (Jellyfin, Kodi, mpv и т.п.):**
Если в превью Hyperion/WLED/HyperHDR на месте 4K/HDR-видео — чёрный прямоугольник, хотя 1080p в том же плеере захватывается нормально — это **не DRM**. На Android 4K-контент (особенно HEVC 10-bit / HDR10 / Dolby Vision) часто рендерится через **защищённый аппаратный overlay** в обход SurfaceFlinger. API `MediaProjection` видит только то, что собрал SurfaceFlinger, поэтому область overlay приходит сплошным чёрным. Это ограничение платформы — оно затрагивает любые ambient-light приложения на Android (Hyperion Android, Lightpack apps и т.д.), фикса на стороне приложения нет.
